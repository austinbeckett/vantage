// =============================================================================
// Form Autocomplete (Multi-Select)
// =============================================================================
// Multi-select component for selecting pharmaceutical forms from Health Canada reference data
// Fetches all forms on first focus and filters client-side

import { useState, useRef, useEffect, useMemo } from 'react'
import { ChevronDown, Loader2, X, FlaskConical, Check } from 'lucide-react'
...
export function FormAutocomplete({
  ...
  // Fetch all forms ONLY after user interacts (lazy load to avoid blocking other requests)
  const { data: forms, isLoading, isFetching } = useAllForms(hasInteracted)
          ...
          >
            ...
            <div className="max-h-48 overflow-y-auto">
              {suggestions.map((form, idx) => {
                const isSelected = selectedSet.has(form)
                return (
                  <button
                    key={`${form}-${idx}`}
                    type="button"
                  ...
                  >
                    ...
                    <FlaskConical className="w-3.5 h-3.5 text-lavender-500 shrink-0" />
